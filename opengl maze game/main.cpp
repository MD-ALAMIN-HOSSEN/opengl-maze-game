...
void initializeCoins() {
    ...
    // Add more coins with specific coordinates as needed
}
//////////////////////////////////////////////////
// player life count
int lives=3;
bool gameOver = false;// this is to track the enemey hit
bool gameEnd = false; // this is to check if all the live is gone
...
// narow way enemy
float obstacleX11 = 555.0f;
float obstacleY11 = 220.0f;
float obstacleWidth11 = 10.0f;
float obstacleHeight11 = 20.0f;
...
void checkEnd() {
  float distance = sqrt(pow(ballX - ballX1, 2) + pow(ballY - ballY1, 2));
...
//Moving Block
  glColor3f(0.3f, 0.5f, 0.7f);
    ...
    glEnd();
             ...
             glColor3f(1.0f, 0.5f, 0.0f);
    glBegin(GL_QUADS);
    glVertex2f(obstacleX11 , obstacleY11 );
    glVertex2f(obstacleX11  + obstacleWidth11 , obstacleY11 );
    glVertex2f(obstacleX11  + obstacleWidth11 , obstacleY11  + obstacleHeight11 );
    glVertex2f(obstacleX11 , obstacleY11  + obstacleHeight11 );
    glEnd();
...
// Draw the coins
  ...
  if(gamegoalStatus){
        ...
        drawText("You have reached your Goal.!", 350, 350); // Position of the text
        std::string text2 = "Score : " + intToString(score);
        drawText(text2.c_str(), 350, 300);

  }

   if (lives < 0) {
        glColor3f(1.0f, 0.0f, 0.0f); // Red color
        drawText("Game Over!", 350, 350); // Position of the text
        std::string text2 = "Score : " + intToString(score);
        drawText(text2.c_str(), 350, 300);
    }
    ...
    if (lives > 0) {
        ...
        std::string text1 = "Lives : " + intToString(lives);
...
void drawHeart(float x, float y, float size) {
    glBegin(GL_TRIANGLE_FAN);
    for (int i = 0; i <= 360; i++) {
        float t = i * 3.1415926 / 180;
        float xOffset = size * 16 * sin(t) * sin(t) * sin(t);
        float yOffset = size * (13 * cos(t) - 5 * cos(2 * t) - 2 * cos(3 * t) - cos(4 * t));
...
//back from top bottom
 ...
 if (ballX + ballRadius > wallX3 && ballX - ballRadius < wallX3 + wallWidth3 &&
  ...
  if (ballX + ballRadius > wallX6 && ballX - ballRadius < wallX6 + wallWidth6 &&
    ...
    if (ballY + ballRadius > wallY8 && ballY - ballRadius < wallY8 + wallHeight8/ 2.0f) {
      ballY -= speed;
      //std::cout << "wall8 bally- !"<< std::endl;
    } else {
      ballY += speed;
      //std::cout << "wall8 bally+ !" <<std::endl;
    }
     //std::cout << "wall8 x !" <<ballX<< "  y"<<ballY << std::endl;
  }
 ...
 //back from top bottom
    ...
    if (ballX + ballRadius > wallX22 && ballX - ballRadius < wallX22 + wallWidth22 &&
     ...
     // std::cout << "wall 22 triger y:"<<  ballY + ballRadius<< wallY22<< std::endl;
   // std::cout << "wall 22 triger x:"<<  ballX + ballRadius<< wallX22<< std::endl;
...
//enemy block colltion check funtion
 void enemycolitionCheck(){

//Collision of obstacles

    if (ballX + ballRadius > obstacleX11  && ballX - ballRadius < obstacleX11  + obstacleWidth &&
        ballY + ballRadius > obstacleY11  && ballY - ballRadius < obstacleY11  + obstacleHeight) {
        gameOver = true;
        //std::cout << "Game Over!" << std::endl;
    }
...
void update(int value) {
     ...
     if (gameOver) {
        lives-=1;
        respawn();//respowan if enemy is hit
        //std::cout << "player lives :"<<lives << std::endl;
    ...
    enemycolitionCheck();//check enemy colletions
    ...
    for (auto& health : healths) {
        if (health.active) { // Check only active health objects
            ...
            lives += 1; // Increase health by 10 (adjust as needed)
...
int main(int argc, char** argv) {
  ...
  initializeCoins(); // Initialize coins
  initializeHealths();// Initialize
